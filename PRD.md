...
**Build:** Hypothetical Document Embeddings - generate a hypothetical answer passage before embedding the query. Also includes query expansion (multiple reformulations for better recall).

**Learn:** Why raw questions don't match document embeddings well, HyDE technique, query-document vocabulary gap, query expansion strategies

---

## Module 10: Contextual Chunk Embeddings

**Build:** Generate a document-level context sentence during ingestion, prepend it to each chunk before embedding. Raw chunk content stored for display, contextual version used for embedding only.

**Learn:** Why isolated chunks lose meaning, contextual embeddings technique, embedding-time vs query-time enhancement tradeoffs

---

## Module 11: Retrieval Evaluation Framework

**Build:** Auto-generate eval cases from documents (LLM creates questions from chunks), measure recall@5, recall@10, MRR. Admin-only API endpoint. Supports both auto-generated and custom test cases.

**Learn:** Retrieval metrics (recall, MRR), why measurement drives improvement, eval-driven tuning of chunking/embedding/search parameters
